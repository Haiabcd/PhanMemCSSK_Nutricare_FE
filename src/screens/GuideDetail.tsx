...
export type ContentDetailParam = RecipeItem | ArticleItem | VideoItem;
...
function MacroPill({ m }: { m: Macro }) {
  ...
  return (
    <ViewComponent
      variant="card"
      p={10}
      style={[s.macroPill, { borderColor: C.border, backgroundColor: C.bg }]}
    >
  ...
  return (
    <View style={s.imageWrap}>
      ...
      {!!onAddToggle && (
        <Pressable
          onPress={onAddToggle}
          style={[s.addCircle, isAdded && s.addCircleActive]}
        >
          <Entypo
            name={isAdded ? 'check' : 'plus'}
            size={20}
            color={isAdded ? C.onPrimary : C.primary}
          />
        </Pressable>
      )}
    </View>
    ...
    <Container>
      ...
      >
        ...
        <HeroMedia
          ...
          onAddToggle={
            data.type === 'recipe' ? () => setIsAdded(v => !v) : undefined
          }
          isAdded={isAdded}
        />
          ...
          <>
            ...
            {!!(data as RecipeItem).videoUrl && (
              <ViewComponent variant="card" p={20} mb={24} style={s.card}>
                ...
                <Pressable
                  onPress={() =>
                    Linking.openURL((data as RecipeItem).videoUrl!)
                  }
                  style={s.videoThumb}
                >
        ...
        {data.type === 'article' && (
          <ViewComponent variant="card" p={20} mb={24} style={s.card}>
            ...
            */}
            <TextComponent
              mt={10}
              text={(data as ArticleItem).contentText ?? '...'}
            />
          </ViewComponent>
        )}
        ...
        {data.type === 'video' && (
          <ViewComponent variant="card" p={20} mb={24} style={s.card}>
            ...
            <TextComponent
              mt={8}
              text={(data as VideoItem).description ?? '...'}
            />
          </ViewComponent>
        )}
      </ScrollView>
