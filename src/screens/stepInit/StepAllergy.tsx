...
const SUGGESTED_ALLERGIES = [
  "Hải sản","Tôm","Cua","Cá biển","Sữa bò","Trứng",
  "Đậu phộng","Hạt cây","Đậu nành","Lúa mì","Gluten",
  "Phấn hoa","Bụi nhà","Động vật","Thuốc","Mỹ phẩm",
];
  ...
  return (
    <View pointerEvents="none" style={[StyleSheet.absoluteFill, { alignItems: "flex-end" }]}>
      <View style={[styles.sbTrack, { height: trackH }]}>
        <View style={[styles.sbThumb, { height: thumbH, transform: [{ translateY: thumbY }] }]} />
      </View>
    </View>
  );
};

export const StepAllergiesScreen = () => {
  ...
  const existsInsensitive = (v: string) =>
    form.allergies.some(a =>
      a.localeCompare(v, undefined, { sensitivity: "accent" }) === 0
    );

  const handleAdd = (val?: string) => {
    const v = (val ?? text).trim();
    if (!v) return;
    if (existsInsensitive(v)) { setText(""); Keyboard.dismiss(); return; }
    addAllergy(v);
    setText("");
    Keyboard.dismiss();
  };
  ...
  const onSugLayout = (e: LayoutChangeEvent) => setSugVisibleH(Math.min(SUG_MAX, Math.round(e.nativeEvent.layout.height)));
  const onSelLayout = (e: LayoutChangeEvent) => setSelVisibleH(Math.min(SEL_MAX, Math.round(e.nativeEvent.layout.height)));
  ...
  const onSugScroll = (e: NativeSyntheticEvent<NativeScrollEvent>) => setSugScrollY(e.nativeEvent.contentOffset.y);
  const onSelScroll = (e: NativeSyntheticEvent<NativeScrollEvent>) => setSelScrollY(e.nativeEvent.contentOffset.y);
    ...
    >
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={styles.sectionLabel}>Gợi ý phổ biến ({suggestions.length})</Text>
          </View>
              ...
              >
                {suggestions.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.suggestText, selected && { color: GREEN_DARK }]}>{s}</Text>
                    </Pressable>
                  );
                })}
              </ScrollView>
        ...
        <View style={[styles.card, { flex: 1 }]}>
          <View style={styles.cardHeader}>
            <Text style={styles.sectionLabel}>Dị ứng đã chọn ({form.allergies.length})</Text>
            {!!form.allergies.length && (
              <Pressable onPress={clearAllergies} style={({ pressed }) => pressed && styles.pressed}>
          ...
          {form.allergies.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.emptySub}>Nhập dị ứng hoặc chọn từ gợi ý bên trên.</Text>
            </View>
          ) : (
...
const styles = StyleSheet.create({
  ...
  sbThumb: {
    ...
    shadowColor: GREEN,
    ...Platform.select({
      ios: { shadowOpacity: 0.35, shadowRadius: 3, shadowOffset: { width: 0, height: 0 } },
      android: { elevation: 2 },
    }),
  },
  ...
  emptyText: { fontSize: 15, color: SLATE_600, fontWeight: "600", marginBottom: 4 },
